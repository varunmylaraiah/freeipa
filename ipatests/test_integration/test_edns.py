...
class TestDNSOverTLS(IntegrationTest):
    ...
    def test_install_dnsovertls_without_setup_dns_master(self):
        """
        This test installs an IPA server using the --dns-over-tls option without using setup-dns option, and captures warnings that appear.
        """
    ...
    def test_validate_DoT_options_master(self):
        ...
        assert '''--dns-over-tls-cert=DNS_OVER_TLS_CERT
                        ...
                        certificate will be requested from IPA CA''' in cmdout.stdout_text
        assert '''--dns-over-tls-key=DNS_OVER_TLS_KEY
                        Key for certificate specified in --dns-over-tls-cert''' in cmdout.stdout_text
        assert '''--dns-over-tls      Configure DNS over TLS''' in cmdout.stdout_text
